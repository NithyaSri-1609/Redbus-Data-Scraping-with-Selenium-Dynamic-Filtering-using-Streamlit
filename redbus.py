...
df['AC/Non-AC'] = df['Bus Type'].apply(lambda x: 'Non-A/C' if 'Non A/C' in x or 'Non A.C' in x or 'Non AC' in x else 'A/C')
df['Seater/Sleeper'] = df['Bus Type'].apply(lambda x: 'Seater' if 'Seater' in x else 'Sleeper' if 'Sleeper' in x else 'Seater/Sleeper')
...
if st.session_state.page == 'home':
    ...
    with col3:
        st.session_state.start_time = st.selectbox('BUS STARTING TIME', options=[''] + df['Departure time'].unique().tolist(), index=0)
    with col4:
